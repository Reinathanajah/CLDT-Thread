    ...
    const cldt_control_profile_t *profile)
{
     ...
     * 2. Require resolved_digest to contain at least one nonzero byte. Digest
     ...
     * 3. Require nonzero observation age, trust-window count, TTL, rate ceiling,
     *    critical period, and bulk burst limit. Check any derived arithmetic
     *    with overflow-safe operations before returning success.
     * 4. Keep this function deterministic and side-effect free so a profile
